#!/usr/bin/env python3
"""
Chan    max_releases: int = typer.Option(
        5,
        "--max-releases", "-n",
        help="Maximum number of releases to fetch per project"
    )
): - A tool to fetch and display release notes from popular open-source projects.
"""
...
def crawl(
    output_dir: Path | None = typer.Option(
        Path(), "--output-dir", "-o", help="Output directory for generated files"
    ),
    ...
    console.print(
        f"Fetching last {max_releases} releases for {len(PROJECTS)} projects..."
    )
        ...
        for project_name, repo_info in PROJECTS.items():
            ...
            try:
                releases = crawler.get_releases(
                    repo_info["owner"], repo_info["repo"], max_releases
                )
                ...
                console.print(
                    f"SUCCESS {project_name}: {len(releases)} releases fetched"
                )
    ...
    if output_dir == Path():
        console.print(
            "[dim]Note: Files generated in root directory are ready for GitHub Pages deployment.[/dim]"
        )
...
def list_projects():
    ...
    for project_name, repo_info in PROJECTS.items():
        ...
        console.print(
            f"   URL: https://github.com/{repo_info['owner']}/{repo_info['repo']}"
        )
